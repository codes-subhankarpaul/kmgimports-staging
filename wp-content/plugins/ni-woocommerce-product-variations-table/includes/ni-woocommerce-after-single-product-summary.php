...
if ( !class_exists( 'Ni_wooCommerce_After_Single_Product_Summary' ) ) { 
	class Ni_wooCommerce_After_Single_Product_Summary{
		...
		function get_default_columns(){
            ...
            $columns["variation_description"] = __('Description', 'nipvt');
				...
				?>
                 <table  cellspacing="0" class="nipv_table tablesorter" style="width:100%" id="nipv-tablesorter">
                            ...
                            <!--<th  style="width:10%">Image</th>
                             ...
                             <?php foreach($columns  as $key=>$value): ?>
                                <?php switch ($key) {
                                    ...
                                    case "stock_quantity":
                                    ?>
                                     <th class="{sorter: 'digit'}"  style="width:5%;"><?php echo $value; ?></th>
                                    <?php	
                                    break;
                                    ...
                                    case "quantity":
                                    ?>
                                     <th style="width:10%;text-align:center" data-sorter="false"><?php echo $value; ?></th>
                                    <?php	
                                    break;
                                    case "add_to_cart_column":
                                    ?>
                                     <th data-sorter="false" class="add_to_cart_column" style="width:20%; text-align:right"><?php echo $value; ?></th>
                                    <?php	
                                    break;
                                } ?>
                    ...
                    <tbody>
                        ...
                        foreach ($available_variations as $key => $value) {
						    $product_variation = wc_get_product($value['variation_id']); //echo '<pre>'; print_r($product_variation); echo '</pre>';
                        	...
                        	?>
                                ...
                                foreach ($columns as $k => $v){
                                    switch ($k) {
                                        ...
                                        case "variation_description":
                                            ?>
                                                <td valign="middle"><?php if($product_variation->get_variation_description() != '' ){ echo esc_html($product_variation->get_variation_description()); } else { echo esc_html( $product->get_title()).'---'.esc_html(implode(" - ", $all_variation)); } ?></td>
                                            <?php
                                            break;
                                        case "quantity":
                                            ?>
