/* global kmgimportGetHexLum, jQuery */
( function() {
	// Add listener for the "background_color" control.
	wp.customize( 'background_color', function( value ) {
		value.bind( function( to ) {
			var lum = kmgimportGetHexLum( to ),
				isDark = 127 > lum,
				...
				stylesheetID = 'kmgimport-customizer-inline-styles',
				stylesheet,
				styles;
			...
			if ( ! stylesheet.length ) {
				jQuery( '#kmgimport-style-inline-css' ).after( '<style id="' + stylesheetID + '"></style>' );
				stylesheet = jQuery( '#' + stylesheetID );
			}
